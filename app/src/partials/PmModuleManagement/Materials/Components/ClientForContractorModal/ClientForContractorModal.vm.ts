...
import { E, Env, ErrorListHolder, lang, restQuery, T, utils } from '~/api';
...
export class ClientForContractorModalVm {
    ...
    constructor(private parentVm: MaterialsVm) {
    ...
    get canContractorConfirm() {
        ...
        const { materialUserTaskType, canBeSkipped, skipTaskObject } = this.materialProgress.currentTask;
    ...
    get itemsRequestedByContractor() {
        ...
        return this.materialProgress.submissions
            .flatMap(submit => {
                if (submit.materialSubmissionType === E.MaterialSubmissionType.materialQuantitiesRequest) {
                    return submit.taskUpdates as TaskUpdateType[];
                }
    ...
    get clientPurchase() {
        ...
        return this.materialProgress.submissions
            .flatMap(submit => {
                if (submit.materialSubmissionType === E.MaterialSubmissionType.materialsPurchased) {
                    return submit.taskUpdates as TaskUpdateType[];
                }
    ...
    mount = async () => {
        ...
        const { currentClientForContractorMaterial } = this.parentVm;

        if (!currentClientForContractorMaterial) {
            return;
        }
        ...
        const result = await restQuery.getMaterialProgress(
            currentClientForContractorMaterial.materialWorkflowId,
        );
    ...
    submit = (isApproved = true) => {
        (async () => {
            ...
            if (this.isCanAddItems) {
                const result = await restQuery.addSubmitMaterialsRequests(this.materialProgress);
                ...
                runInAction(() => {
                    ...
                    this.parentVm.updateStatusByClientForContractor(result.currentTask);
                    this.materialProgress = result;
                });
            ...
            if (this.isCanAddPurchase) {
                ...
                runInAction(() => {
                    ...
                    if (response) {
                        this.parentVm.updateStatusByClientForContractor(response.currentTask);
    ...
    close = () => {
        this.parentVm?.closeClientForContractorModal();
    ...
    openCommentImage = (file: FileDataType) => {
